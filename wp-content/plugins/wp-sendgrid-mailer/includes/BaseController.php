...
class BaseController
{
	...
	{
		$data_to_store = array();
		$email_service = sanitize_text_field($data['email_service']);
		if(!empty($email_service) && array_key_exists($email_service, BaseController::$available_service))
		{
			...
			$header_info = array('from_name' => sanitize_text_field($data['from_name']), 'from_email' => sanitize_email($data['from_email']));
