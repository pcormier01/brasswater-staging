    ...
    private function prepare_where_condition()
    {
        ...
        if($_POST)
        {
            if(isset($_POST['service']) && !empty($_POST['service'])){
                $email_service = sanitize_text_field($_POST['service']);
                $where_query .= " email_service = '{$email_service}' and";
            }
        ...
        switch( $column_name ) {
            ...
            case 'status':
                if($item->$column_name == 'Failed') {
                    echo "<span class = 'label label-danger'> Failed </span>  <span class='dashicons dashicons-info' data-toggle='tooltip' data-placement='bottom' title = '".esc_attr($item->message)."'></span>";
                }
