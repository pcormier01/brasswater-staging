		...
		function column_default( $item, $column_name ) {
			...
			if ( ! empty( $this->currenttimestamp_field ) && $column_name == $this->currenttimestamp_field ) {
				...
				foreach ( $this->actions as $action ) {
					...
					$actions[ $action_slug ] = sprintf( '<a href="javascript:void(0);" data-item-id="' . $item->{$this->primary_col} . '" data-page-slug="'.$this->admin_listing_page_name.'" data-record-type="'.$this->primary_col.'" data-action="delete" data-toggle="modal" data-target="#delete_fc_record">' . $action_label . '<i class="fa fa-trash"></i></a>', $item->{$this->primary_col} );
