...
(function($){
    ...
    function moveSectionToTop(item, callback) {
        ...
        item.css('z-index', 1000).css('position', 'relative').slideUp( 400, function () {
            ...
            $("html, body").animate({ scrollTop: $('#visual_composer_content').offset().top -200 }, 600, function(){
    ...
    function moveSectionToBottom(item, callback) {
        ...
        item.css('z-index', 1000).css('position', 'relative').slideUp( 400, function () {
            ...
            $("html, body").animate({ scrollTop: $('#visual_composer_content')[0].scrollHeight }, 600, function(){
    ...
    $(document).on('click', '.vc_column-move-bottom', function(event) {
        ...
        moveSectionToBottom( $section, function(){
    ...
    $(document).on('click', '.vc_column-move-top', function(event) {
        ...
        moveSectionToTop( $section, function(){
...
function ut_check_unique_id( params ) {
        ...
        vc.element_start_index = 0, vc.AddElementUIPanelBackendEditor = vc.PanelView.vcExtendUI(vc.HelperPanelViewHeaderFooter).extend({
            ...
            createElement: function(e) {
                ...
                if (e && e.preventDefault && e.preventDefault(), tag = ($control = $(e.currentTarget)).data("tag"), row_params = {}, column_params = {
                    ...
                    if (vc.storage.lock(), "vc_section" === tag || "ut_content_block" === tag ) {
                        var modelOptions = {
                            shortcode: tag
                        };
    ...
    function($) {
        ...
        vc.edit_form_callbacks = [], vc.atts = {
            ...
            render: function(param, value) {
                ...
                return "external_link" === this.model.getParam("source") && (value = this.model.getParam("custom_srcs")), _.isEmpty(value) ? (this.$el.removeData("field-" + param.param_name + "-attach-images"), vc.atts.attach_images.updateImages($thumbnails, "")) : $.ajax({
                    ...
                    data: {
                        ...
                        content: value,
                        limit: $thumbnails.hasClass('ut-attachment-list-builder'),
                        _vcnonce: window.vcAdminNonce
                    },
                    dataType: "html",
                    context: this
            ...
            render: function(param, value) {
                ...
                if ( "image" === param.param_name || "logo" === param.param_name ) switch (this.model.getParam("source")) {
                    case "external_link":
            ...
            extractValues: function(param, $el) {
                ...
                return data = [], self = this, $el.each(function() {
                    ...
                    innerData = {}, self.$content = $(this), _.each(param.params, function(par) {
                        ...
                        innerParamName = (innerParam = $.extend({}, par)).param_name,
                        innerParam.param_name = param.param_name + "_" + innerParamName,

                        ((value = vc.atts.parse.call(self, innerParam)).length || innerParam.save_always) && (innerData[innerParamName] = value)
                ...
                $(".vc_color-control", $field).each(function() {
                    ...
                    value.match(/rgba\(\d+\,\d+\,\d+\,([^\)]+)\)/) && (alpha_val = 100 * parseFloat(value.match(/rgba\(\d+\,\d+\,\d+\,([^\)]+)\)/)[1])), $control.wpColorPicker({
                        ...
                        change: _.debounce(function() {
                            $(this).trigger("change")
                        }, 500) // @United Opacity Colorpicker
    ...
    function($) {
        vc.clone_index = 1, vc.shortcode_view = Backbone.View.extend({
            ...
            cloneModel: function(model, parent_id, save_order) {
                ...
                return new_order = _.isBoolean(save_order) && !0 === save_order ? model.get("order") :

                    parseFloat(model.get("order")) + vc.clone_index,
                    params = ut_check_unique_id( _.extend({}, model.get("params")) ), // @United
        ...
        window.VcRowView = vc.shortcode_view.extend({
            ...
            convertRowColumns: function(layout) {
                ...
                return _.each(layout_split, function(value, i) {
                    ...
                    var new_column_params,
                        new_column,
                        column_data = _.map(value.toString().split(""), function(v, i) {
                            return parseInt(v, 10)
                        });

                    new_width = 3 < column_data.length ? column_data[0] + "" + column_data[1] + "/" + column_data[2] + column_data[3] : 2 < column_data.length ? column_data[0] + "/" + column_data[1] + column_data[2] : column_data[0] + "/" + column_data[1],
                    ...
                    new_layout.push(new_width), new_column_params = _.extend(_.isUndefined(columns[i]) ? {} : columns[i].get("params"), {
                        width: new_width
            ...
            loadTinySource: function( original, $column ) {
                ...
                http.onreadystatechange = function() {
                    ...
                    $('<span class="vc_row_image" style="background-image: url(' + preview_url + ');" title="' + i18nLocale.column_background_image + '"></span>').insertAfter($column);
            ...
            buildDesignHelpers: function() {
                ...
                image && this.loadTinySource(image, $elementToPrepend),
            ...
            loadTinySource: function( original, $column ) {
                ...
                http.onreadystatechange = function() {
                    ...
                    $('<span class="vc_column_image" style="background-image: url(' + preview_url + ');" title="' + i18nLocale.column_background_image + '"></span>').insertBefore($column);
            ...
            buildDesignHelpers: function() {
                ...
                image && this.loadTinySource( image, $column_toggle ),
            ...
            loadTinySource: function( original, $column ) {
                ...
                http.onreadystatechange = function() {
                    ...
                    $column.next('.vc_row_image').remove();
                    $('<span class="vc_row_image" style="background-image: url(' + preview_url + ');" title="' + i18nLocale.column_background_image + '"></span>').insertAfter($column);
            ...
            changedContent: function(view) {
                ...
                if (this.$tabs.hasClass("ui-tabs") || (this.$tabs.tabs({
                    ...
                    view.model.get("cloned_from");
                    var $tab_controls = $(".tabs_controls > .add_tab_block", this.$content),
            ...
            deleteShortcode: function(e) {
                ...
                if (this.model.destroy(), !vc.shortcodes.where({
                    parent_id: parent_id
                }).length) return vc.shortcodes.get(parent_id).destroy(), !1;
                var params = this.model.get("params"),
    ...
    function($, _) {
            ...
            $.ajax({
                ...
                if ("function" == typeof callback) {
                    ...
                    if( response.data.ids.length ) {
                        ...
                        var query = wp.media.query({ post__in: response.data.ids, posts_per_page: -1, orderby: 'date' }),
                            promise = query.more();

                        promise.done( function() {

                            callback( query.models );
                        });
        ...
        attachmentCompatRender = _.extend(media.view.AttachmentCompat.prototype.render), media.view.AttachmentCompat.prototype.render = function() {
            ...
            return attachmentId = this.model.get("id"), attachmentCompatRender.call(this), _.defer(function() {
                ...
                if( that.controller.$el.find(".attachment-info").prev('.vc-image-filter-setting').length ) {
                    return;
                }
            ...
            updateSelection: function() {
                ...
                var attachment_ids = ids.toString().split(/,/);

                var query = wp.media.query({ post__in: attachment_ids, posts_per_page: -1, orderby: 'date' }),
                    promise = query.more();

                promise.done( function() {

                    if( query.length && query.models !== undefined ) {

                        selection.reset(query.models)
            ...
            processImages(_.extend({}, selection), function(newAttachments) {
