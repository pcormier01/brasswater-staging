 ...
 * @return string
 */
function fieldAttachedImages( $images = array(), $limit = false ) {

	$output = '';

	// @United add limitation
	$counter = 0;
	...
	foreach ( $images as $image ) {
		...
		if( $limit && $counter == 20 ) {
			...
			$output .= '
			<li class="added">
				<a href="#" class="vc_control-btn vc_control-btn-edit">...</a>
			</li>';
			break;
		...
		if ( is_numeric( $image ) ) {
			...
			$thumb_src = wp_get_attachment_image_src( $image, 'ut-mini' );
...
function vc_convert_shortcode( $m ) {
	...
	//@United
	if ( 'vc_row' === $tag || 'vc_section' === $tag || 'ut_content_block' === $tag ) {
		return $output;
	}
