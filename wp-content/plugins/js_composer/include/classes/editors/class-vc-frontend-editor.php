...
class Vc_Frontend_Editor implements Vc_Editor_Interface {
	...
	function renderEditButton( $link ) {
		...
		if ( $this->showButton( get_the_ID() ) ) {
			return $link . ' <a href="' . self::getInlineUrl() . '" id="vc_load-inline-editor" class="vc_inline-link">' . __( 'Edit with Page Builder', 'js_composer' ) . '</a>';
		}
	...
	function renderRowAction( $actions ) {
		...
		if ( $this->showButton( $post->ID ) ) {
			...
			$actions['edit_vc'] = '<a href="' . admin_url('post.php?action=edit&post=' . $post->ID ) . '">' . __( 'Edit with Page Builder', 'js_composer' ) . '</a>';
			//$actions['edit_vc'] = '<a href="' . $this->getInlineUrl( '', $post->ID ) . '">' . __( 'Edit with Page Builder', 'js_composer' ) . '</a>';
