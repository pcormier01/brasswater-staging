...
class Vc_Add_Element_Box implements Vc_Render {
	...
	public function renderButton( $params ) {
		...
		// @United
		$united_themes_base_url = '';

		$output .= '<li data-element="' . $params['base'] . '"'. ( isset( $params['presetId'] ) ? ' data-preset="' . $params['presetId'] . '"' : '' ) .' class="wpb-layout-element-button vc_col-xs-12 vc_col-sm-4 vc_col-md-3 vc_col-lg-2' . ( isset( $params['deprecated'] ) ? ' vc_element-deprecated' : '' ) . $category_css_classes . $class_out . '"' . $data . '>';
		$output .= '<div class="vc_el-container">';
		$output .= '<a id="' . $params['base'] . '" data-tag="' . $params['base'] . '" class="dropable_el vc_shortcode-link' . $class . '" href="#" data-vc-clickable>';
		$output .= $this->getIcon( $params ) . $name . $description;
		$output .= '</a>';
		$output .= '</div>';

		if( !empty( $params['preview_slug'] ) ) {
			$output .= '<a class="ut-ui-button ut-ui-button-module-preview">' . esc_html( 'Preview Module', 'js_composer') . '</a>';
		}
